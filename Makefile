...
ENV_FILE = hadoop.env
current_branch := $(shell git rev-parse --abbrev-ref HEAD | sed 's/heads\///g')
build:
...
wordcount:
	...
	docker run --network ${DOCKER_NETWORK} --env-file ${ENV_FILE} bde2020/hadoop-base:$(current_branch) hdfs dfs -copyFromLocal -f /opt/hadoop-3.3.0/README.txt /input/
